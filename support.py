...
def emoji_rank(selected_user, df):
    ...
    for idx, row in df2.iterrows():
        # The findall() is a functions for regex that help to find a matches
        emojis_found = r.findall(row["message"])
        for emoji_found in emojis_found:
            emoji_ctr[emoji_found] += 1

    # The reason to use pd.dataframe is we want to put the emojis_df into the dataframe tables
